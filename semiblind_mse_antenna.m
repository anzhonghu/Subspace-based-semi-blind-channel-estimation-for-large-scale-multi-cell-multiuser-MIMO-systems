...
K = 3;%user number
tao = K;%pilot length
r = 1000;%center to vertex distance(m)
rc = r * 0.8;
...
for ii = 1 : ant_n
    ...
    for jj = 1 : Num
        ...
        for j = 1 : L
            ...
            for k = 1 : K
                ...
                R_temp = R_u;
                for j1 = 1 : K
                    R_temp = R_temp + beta * (H_e_pilot(:,(j-1)*K+j1) * H_e_pilot(:,(j-1)*K+j1)');
                end
                H_e_null(:,(j-1)*K+k) = beta * i_ant * (R_temp \ H_e_pilot(:,(j-1)*K+k));
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    cova_store(ii,1:5) = [csi_mse,bound1];
    sprintf('%d',ii)
end
...
set(h,'PaperType','A4');
xx = axes('FontSize',16);
semilogy(ant_s,cova_store(:,4),'b-.o','MarkerSize',10)%no difference for different antenna numbers
hold on
semilogy(ant_s,cova_store(:,1),'k:*')
semilogy(ant_s,cova_store(:,2),'k:s','MarkerSize',10)
semilogy(ant_s,cova_store(:,3),'k-d','MarkerSize',10)
semilogy(ant_s,cova_store(:,5),'r-+')
le = legend('Pilot-based estimator','Estimator in [7]','Estimator in [8]','Proposed estimator','CRB','Location','Northeast');
set(le,'Fontsize',10,'Fontname','Times')
xlabel('Number of BS antennas','Fontsize',16,'Fontname','Times')
ylabel('MSE','Fontsize',20,'Fontname','Times')
%print(h,'-dpdf','evd_mse_antenna')